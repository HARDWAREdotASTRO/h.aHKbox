...
void loop() {
 /* while (! nau.available()) {
  ...
  while (tnow2 < tend2) {
    ...
    ReadSum = ReadSum + (voltReading * voltReading);
  ...
  I = 0.3 / (973000.0 + Rlast);
  VoltResReading = sqrt(ReadSum / MeasCount); //This is RMS
  Serial.println(VoltResReading, 8); 
  R = VoltResReading / I;
  Rlast = R;
  Serial.println(R);
  Serial.println();
