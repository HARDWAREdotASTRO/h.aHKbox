...
int window_length = (period * Num_Cycles) / 0.0011627907; //integer calculated to provide the # of frames, or readings, that will be used per RMS calculation (SPS/Read Out Rate)
...
void setup() {
  ...
  ads.startADCReading(ADS1X15_REG_CONFIG_MUX_SINGLE_0, 1);
...
void loop() {
  ...
  */
  ...
  //if ((tim * 1000000) - tim_old >= 1100) //trying to get to 6100 microseconds or 6.100 millis between readings
  //{
  if (frame <= window_length)
  {

    val = ads.getLastConversionResults();
    rms_sum = rms_sum + sq(ads.computeVolts(val));
    frame = frame + 1; // Update frame # for next ADC measurement
    //Serial.println(ads.computeVolts(val), 5);
  ...
  //}
  tim = micros();
  delayMicroseconds((1e6 / 860) - (tim - tim_old) - 6);
  tim_old = micros(); //Calculate Micros
  //Serial.println(1 / ((tim_old - tim_prev) * 1e-6), 6);
  tim_prev = micros();
}
