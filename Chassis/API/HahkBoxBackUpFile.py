           ...
           # print("yes /no value:" + str(yesNo) + " binary:" + bin(yesNo & 0xff) + " \n raw response [0]: \n" + str(response[0]) + " binary: " + bin(response[0] & 0xff)+ " \n raw response[1]: \n" + str(response[1]) + " binary: " + bin(response[1] & 0xff))
            ...
            if(yesNo < 128):
              ...
              #  print(response[1])
                ...
                #information = bytearray(bytes(self.read(a, byteRequest ), int))
                #information = unpack(unpackString, bytes(self.read(a, byteRequest ))) # changed addresses[a] to just 'a' removed: response << 8 from this line also 07-25-22 removed int() from byteRequest
                print(bytes(self.read(a, byteRequest )))
                ...
                #print("this is result value: " + str(result))
               # print("\n")
